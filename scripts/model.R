...
ids = fread('names.train.csv',select=c("V1","V2","V3","V4","V5"))[1:3000]
setnames(ids,"id")
train = cbind(ids,train)
train = merge(labels,train,by="id")

train2 = fread('pca.grey.train.csv',select=c("V1","V2","V3","V4","V5"))
ids2 = fread('names.grey.train.csv',select=c("V1","V2","V3","V4","V5"))[1:3000]
setnames(ids2,"id")
...
#Load orange/yellow pixel count features. 
orange = fread('count_orange.test.csv')
setnames(orange,"orange")
yellow = fread('count_yellow.test.csv')
setnames(yellow,"yellow")
ids = fread('names.transform.test.csv')
setnames(ids,"id")
other = cbind(ids,orange,yellow)
other[,share:=orange/(yellow+1)]
...
rhs.test = model.matrix(~0+.*. ,data=test)
...
#finalforecast
output = data.frame(id=test_ids,genus=0.75*(outpred.rf$bumble)+0.25*(outpred.lasso$bumble))
